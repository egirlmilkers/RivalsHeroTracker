import { SEASON_DATES } from './history'
...
{
	const currentSeason = Object.keys(SEASON_DATES).at(-1)
	...
	const vWeight = vanguards.length > 0
		? Math.round(ROLE_TOTAL_WEIGHT / vanguards.length)
		: 0
	const dWeight = duelists.length > 0
		? Math.round(ROLE_TOTAL_WEIGHT / duelists.length)
		: 0
	const sWeight = strategists.length > 0
	...
	const rouletteData = {
		...
		path: 'kg7-we2', // updates the existing wheel rather than try and make a new one
		shareMode: 'gallery', // lets it be publicly visible
		title: 'Marvel Rivals Roulette',
		description:
			`- All heroes up to Season ${currentSeason}\n- Adjusted weights for each class\n- Colored panels based on in-game colors\n- Hero portraits`
	}
